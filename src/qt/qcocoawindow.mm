...
QCocoaWindow::QCocoaWindow(QWindow *tlw)
    ...
    , m_fakeExposedWindow(false)
    , m_registerTouchCount(0)
{
#ifdef QT_COCOA_ENABLE_WINDOW_DEBUG
    qDebug() << "QCocoaWindow::QCocoaWindow" << this;
    
    printf( "QCocoaWindow::QCocoaWindow\n" );
#endif
...
bool QCocoaWindow::isExposed() const
{
    ...
    // Exposed status is generally deferred to Cocoa. In some cases we
    // want to pretend the window is already exposed:
    if (m_fakeExposedWindow)
        return true;
    return [[m_contentView window] isVisible];
}
