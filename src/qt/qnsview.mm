...
#define QT_COCOA_ENABLE_WINDOW_DEBUG
...
- (void)windowNotification : (NSNotification *) windowNotification
{
    ...
    if (notificationName == NSWindowDidBecomeKeyNotification) {
        ...
        QWindowSystemInterface::handleExposeEvent(m_window, QRegion());
       ...
       qDebug() << "expose false" << m_window->isExposed();
        QWindowSystemInterface::handleExposeEvent(m_window, QRegion());
    } else if ([notificationName isEqualToString: @"NSWindowDidOrderOnScreenAndFinishAnimatingNotification"]) {
        qDebug() << "expose true" << m_window->isExposed();
        QWindowSystemInterface::handleExposeEvent(m_window, QRegion(m_window->geometry()));
    } else {
...
{
    // Qt needs to provide backing store content before the window is shown
    // for the first time. drawRect will be called before the window show
    // notifications are called, so we can detect that case here and send
    // an expose event. This will paint and call flushBackingStore which
    // sets m_backingStore.
    if (!m_backingStore) {
        m_platformWindow->m_fakeExposedWindow = true;
        QWindowSystemInterface::handleExposeEvent(m_window, QRegion(m_window->geometry()));
        QWindowSystemInterface::flushWindowSystemEvents();
        m_platformWindow->m_fakeExposedWindow = false;
    }

    // If that didn't work then at least not crash.
    if (!m_backingStore)
        return;
